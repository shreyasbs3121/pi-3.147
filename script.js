...
function attachCartButtons() {
    ...
    document.querySelectorAll('.product-card').forEach(card => {
        ...
        // buy-now buttons in product cards
        const buyNowBtns = card.querySelectorAll('.buy-now-btn');
        buyNowBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation(); // Prevent triggering product-link modal
                
                const product = {
                    id: card.dataset.id || card.getAttribute('data-product'),
                    name: card.dataset.name || card.querySelector('h3').textContent.trim(),
                    price: parseFloat(card.dataset.price) || 2399, // fallback
                    image: card.dataset.image || card.querySelector('img').src
                };
